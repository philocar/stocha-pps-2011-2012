...
public class CplexEnergieRecours extends PLEnergieRecours {
		...
		try {
			...
			for (int p = 0; p < 7; p++) {
				...
				for (int s = 0; s < 100; s++) {
					...
					IloRange eq = cplex.eq(cplex.sum(fx2,
							cplex.prod(donnees.getTurbinage(), x[p * 5 + 4]),
							cplex.prod(1, yp[p * 100 + s]),
							cplex.prod(-1, ym[p * 100 + s])), donnees
							.getScenario(s).getDemandePeriode(p));
					cplex.add(eq);
				}
				...
				IloRange le = cplex.le(exprxi5, donnees.getVolumeInitial()
						+ sommeApports - donnees.getVolumeMin(p));
				IloRange ge = cplex.ge(exprxi5, donnees.getVolumeInitial()
						+ sommeApports - donnees.getVolumeMax(p));
				cplex.add(le);
				cplex.add(ge);
			}
	...
	public static void main(String[] args) {
		DataRecours data = new DataRecours(
				"Data/Données_Recours_parametres_hydraulique.csv",
				"Data/Données_Recours_capacite_max.csv",
				"Data/Données_Recours_scenarios_demande.csv",
				"Data/Données_Recours_scenarios_coeff_dispo_centrale1.csv",
				"Data/Données_Recours_scenarios_coeff_dispo_centrale2.csv",
				"Data/Données_Recours_scenarios_coeff_dispo_centrale3.csv",
				"Data/Données_Recours_scenarios_coeff_dispo_centrale4.csv");
